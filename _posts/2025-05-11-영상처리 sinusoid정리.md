...
1. [Sinusoids (정현파)](#1-sinusoids-정현파)  
2. [Complex Exponentials (복소지수)](#2-complex-exponentials-복소지수)  
3. [Frequency Spectrum of a Sinusoid](#3-frequency-spectrum-of-a-sinusoid)  
4. [Sum of Sinusoids](#4-sum-of-sinusoids)  
5. [Sum of Complex Exponentials](#5-sum-of-complex-exponentials)  
6. [Frequency Spectrum](#6-frequency-spectrum)  
7. [Analysis & Synthesis](#7-analysis--synthesis)
...
- **A**: 진폭 (Amplitude)  
- **ω**: 라디안 주파수 (Radian frequency), $$ \omega = 2\pi f $$  
- **T**: 주기 (Period), $$ T = \frac{1}{f} $$  
- **φ**: 위상 (Phase), $$ \phi = -\omega t_0 $$
...
$$
X(\omega_0) = \frac{A}{2} e^{j\phi},\quad X(-\omega_0) = \frac{A}{2} e^{-j\phi}
$$
...
실수 신호의 경우, 아래 조건을 만족하는 **켤레 대칭**을 가집니다:
